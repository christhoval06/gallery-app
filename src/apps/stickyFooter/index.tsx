...
const ICON_SIZE = 60;
const SMOOT_ANIMATION_DISTANCE = 60;

export default () => {
  const [bottomActions, setBottomActions] = React.useState<LayoutRectangle | null>(null);
  const scrollY = React.useRef<Animated.Value>(new Animated.Value(0)).current;

  const topEdge = (bottomActions?.y ?? 0) - height + (bottomActions?.height ?? 0);
  const inputRange = [-1, 0, topEdge - SMOOT_ANIMATION_DISTANCE, topEdge, topEdge + 1];
      ...
      {bottomActions && (
        <Animated.View
          ...
          <View style={{flexDirection: 'row', height: 60, alignItems: 'center', justifyContent: 'center'}}>
            <View style={{width: 24, height: 24, borderRadius: 24, backgroundColor: 'black', marginHorizontal: 10}} />
            <Animated.Text
              style={{
                  ...
                  transform: [
                    {
                      translateX: scrollY.interpolate({
                        inputRange,
                        outputRange: [ICON_SIZE, ICON_SIZE, ICON_SIZE, 0, 0],
                      }),
                    },
                  ],
                },
              ]}>
              <View style={{width: 24, height: 24, borderRadius: 24, backgroundColor: 'green', marginHorizontal: 10}} />
            </Animated.View>
            <Animated.View
              style={[
                styles.icon,
