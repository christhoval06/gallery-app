...
export default () => {
  ...
  const [duration, setDuration] = React.useState<number>(timers[0]);

  const inputRef = React.useRef<TextInput>(null);
  const textInputAnimation = React.useRef<Animated.Value>(new Animated.Value(timers[0])).current;
  const timerAnimation = React.useRef<Animated.Value>(new Animated.Value(height)).current;
  const buttonAnimation = React.useRef<Animated.Value>(new Animated.Value(0)).current;
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View
        ...
        <TouchableOpacity activeOpacity={0.8} onPress={animation}>
